        ...
        if len(pers_clusters[pers]) > min_frames:
            ...
            for sample in pers_clusters[pers]:
                ...
                if val_count < val_num:  # first 2 images is used as val image
                    ...
                    dst_path = osp.join(train_save_path, str(label_generator))

                if not os.path.isdir(dst_path):
                    os.mkdir(dst_path)
                    # first image is used as val image
                cv2.imwrite(osp.join(dst_path, '{}_{}_{}_1'.format(
                    sample['seq_name'], sample['frame_id'],label_generator)), sample['img'])
                val_count += 1
            label_generator += 1
